...
id 只能由 `[A-Za-z0-9_]` 组成，不能含 `-` 等其他字符。GC 从文本里提取令牌时，是按这个字符集划边界的（见 §5 mark 步骤）：边界之外的字符一律当成令牌已经结束。用带 `-` 的 id（比如直接拿 UUID 当 id）会在第一个 `-` 处被截断，截出来的半截 id 对不上真实 id，对应的图会被判定成没人引用、当孤儿删掉。这条对存量 id 同样适用——超出该字符集的存量 id 读写虽正常，但无法安全参与 GC，接入前必须先迁移。
