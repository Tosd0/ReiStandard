...
于是产出端只有两条路，跟客户端跑在什么设备上无关：**要推就一定弹**（`show: "always"`，嫌打扰配 `tag` 折叠 + `silent`；页面自绘的那类用 `silent: "when-visible"`，前台安静、切后台照响，静不静音由 Service Worker 收到时现算），**不想弹就别推**（落收件箱等补拉）。`"when-hidden"` 算「会弹」照推——它到底弹不弹取决于当下有没有可见窗口，只有 Service Worker 当场知道——但那是给既有部署留的兼容档，新实现在上面两条里挑一个。
