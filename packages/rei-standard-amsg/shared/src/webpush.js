 ...
 * Node 18 deployments must polyfill `globalThis.crypto` from
 * `node:crypto.webcrypto` before importing (amsg-instant 的 `adapters/node`
 * 入口就是干这个的)。
 *
 ...
 *
 * 这是 amsg-instant 与 amsg-server 共用的唯一一份实现 — 两个包的
 * webpush 模块都薄薄地 re-export 这里（server 在其上另加 payload
 * 大小护栏与 scheduled TTL 附录）。实现在独立模块 — shared 内部按
 * 主题拆文件，index 只负责聚合导出。
 */
