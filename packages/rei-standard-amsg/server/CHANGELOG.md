...
- 0822c47: 改任务不再让这次触发重新生成一遍；顶替旧任务在 pg / neon 上也一起成败；投递进行中不接受改排期
...
- 792fe8e: 后台投递少花冤枉钱：LLM 拒了请求不再重试、推送失败只补推送不重新生成、用量按整次 fire 累计
...
- c70cba9: 云端数据可以按角色 / 按命名空间清理，不再只有「整表全清」一个粒度
...
- b09a3bc: 中转站回 200 但响应体是报错时，按 LLM 调用失败处理，不再当成「模型这轮没说话」
...
- Updated dependencies [b09a3bc]
  - @rei-standard/amsg-shared@0.4.0-next.10

## 2.6.0-next.27

### Minor Changes

- 179c4ad: `PUT /client-state` 的 entry 认 `value: null`，表示删掉这个 key
...
- 179c4ad: `client_state` 与 `message_outbox` 的例行清理走索引，不再每分钟全表扫描
...
- cb05c91: `client_state` 的条件写护栏值钳到服务端当前时刻
...
- e254d09: `client_state` 不再被「来自未来」的时间戳锁死
