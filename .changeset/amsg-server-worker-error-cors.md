...
- 回显 Origin 意味着任意第三方页面都能读到这条响应，所以跨域读到的 `error.cause` 只有 `stage` / `name` / `code`，不带 `message`——构建期异常的原文往往就是部署信息本身（`env.DB is undefined` 报的是 binding 名）。同源请求和不带 `Origin` 的调用（`curl`、服务端之间调用）照旧拿全文，`wrangler tail` 里也一直有。`stage: 'request'` 的 500 不受这层影响：那时白名单已经是已知的，谁读得到是部署方批准过的；
