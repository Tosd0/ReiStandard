...
收齐还原了、和中途放弃了（分片对不上、超限、拼不回来），走的是同一套收尾：先写一条 done 墓碑，再清 pending 记录和已收的分片。

墓碑让结论变粘：同一个 id 之后再来分片（推送服务的常规重投）会被直接丢掉，不会重开一份 pending 从零累计——不然 `multipart.maxTotalBytes` 拦下的那份，重投几次就能重新凑齐还原出来。TTL 清扫也认这块墓碑：清理途中万一出错、pending 记录留了下来，清扫看见墓碑就知道这个 id 已经有结论了，不会为一条已经还原并渲染出来的消息再广播一次 `MULTIPART_EXPIRED`。
